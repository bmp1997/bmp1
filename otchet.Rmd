 ...
 **X∼U(5,105)X∼U(5,105)  
 Y=f(X)+ϵY=f(X)+ϵ**  
 где f(x)={8 + 3.5*sin((x - 30)/9)}, ϵ∼N(0,1)ϵ∼N(0,1)
...
Наименьшее значение MSE на тестовой выборке соответствует числу степеней свободы 6 и равно 1.46. Что и является компромиссом между простотой и точностью модели.
...
Теперь изменим исходные данные, n=600.
...
Сравнив полученные модели, можно увидеть, что при увеличении кол-ва набл. лучшая модель ближе к истинной функции, так же значение ошибки MSE на тестовой выборке уменьшается, в данном случае она изменилась с 1.46 до 1.14
